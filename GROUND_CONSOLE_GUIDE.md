...
★ **2026-09-17: 地上局リンクは既定で BLE** (log_recorder 経由) になった。
以下の「地上局 XIAO / IM920 / COM ポート」の説明は `--link im920`
(= `utils/config.py` の `GROUND_LINK_BACKEND = "im920"`) のときの話。
BLE 版では COM ポートは使わず、`S`/`D`/`C` キーは無効 (`Z` は PC 側の受信統計
クリア)、`P`/`k`/`i` は `console.py` から直接送れる。切り替え方は README の
「地上局リンクの経路 (BLE / IM920) の切り替え」を参照。
...
| `S` | 地上局XIAOの状態表示 (地上局 `main.cpp` の `s` を転送) |
