...
// 速度ループ: (目標速度 - 実測速度[m/s]) → 目標リーン角[deg]\n//   kp=6 なら「1 m/s ずれていたら 6度 傾けて戻す」\n//\n//  ★ 2026-09-11: 6.0 -> 4.0 と結論したが、このコード上は 6.0f のまま未適用\n//    だった。LOG0009 (2026-09-14) で fh_leanr/leanp・flow_vx/vy・roll/pitch_ang\n//    が揃って 0.25〜0.32Hz でピークを持ち、以前の 0.31Hz リミットサイクルと\n//    同一と確認 (KP=6.0 のまま飛んでいたので当然)。ここでようやく 4.0 を適用。\n//    LOG0046 の再システム同定で判明: FLOW_POS_KP を 1.0->0.7->0.6 と下げても\n//    一巡ゲイン |L(0.31Hz)| が 0.55 のまま動かなかった。理由は等価PIDで\n//    Kd_eff = FLOW_VEL_KP (FLOW_POS_KP に依存しない) で、0.31Hz は D支配帯域\n//    だから。FLOW_POS_KP を下げると P は下がるが Td=1/FLOW_POS_KP が伸びて\n//    D支配帯では相殺 → |L| 不変。LOG0038/0042/0045 が全部同じ数字だったのは\n//    これ。0.31Hz の |L| を下げるには FLOW_VEL_KP を触るしかない。\n//        6.0 -> 4.0 で |L| 0.55 -> ~0.37 (余裕 ~9dB)。Td (=1/FLOW_POS_KP) は\n//    変わらないので位相形状はそのまま、ゲインだけ純粋に下がる。\n//    ★ 代償: 速度ループ全体が柔らかくなる = 外乱 (押された時) の復帰が遅い。\n//      無風の定点ホバーなら実害は小さいはず。次に飛ばしてまだ荒れるなら\n//      FLOW_VEL_KD を足す (0.31Hz だけ減衰を足せて DC剛性は維持) が次の手。\n//    ★ 姿勢/レートループは無罪 (LOG0037/0046 独立で lean->roll_ang:\n//      ゲイン ~0.93 / 位相 -4〜-5度 = 揺れの周波数で既に透明)。\n//      ループ速度を上げても 0.31Hz に対して余裕が 640〜3200倍あり無意味。
//
//  ★ 2026-09-15: KP 4.0->5.0 / KI 2.0->0.8 (FLOW_POS_KP も 0.6->0.4)。
//    KP=4.0 にしてもホバー中 0.2Hz 前後の揺れが残っていた (9/15 の
//    s5_link 7本の POSHOLD 区間で fh_vxc/vyc sd 0.15〜0.26m/s、目標速度から
//    実測速度が ~1s 遅れて追従)。ログから同定したリーン->加速度は
//    ゲイン 0.14〜0.19 (理論 0.171) / 遅れ 0.45〜0.50s。この遅れを入れた
//    1軸モデルで、現行 (KP4/KI2/POS0.6) は 0.15Hz・1周期で振幅0.3倍しか
//    減らない弱減衰になり、旧 (KP6/KI2/POS1.0) は遅れ0.6sで発散した
//    (周波数はモデルでは0.2Hz付近で、過去の0.31Hzとは一致しない)。
//    揺れの主因は KP ではなく「0.45s の遅れに対して KI=2 (零点 0.5rad/s が
//    交差周波数に重なる) と FLOW_POS_KP=0.6 が効きすぎ」の位相不足と見る。
//    遅れ 0.3〜0.6s のどれでも振動がほぼ出ず、押された後の収束が
//    10〜30s -> 6s 以下になる組み合わせを選んだ (1軸・無風・ノイズ無しの
//    モデルなので、実機での確認が前提)。
...
//   デバイスチェックが OK になる事例があった (実際のレンジングは電源不足で
//   進まず、高度が凍結する)。begin() 内で「起動後この時間内に実サンプルが
//   1回来るか」まで確認することで、電源断をデバイスチェックで検出する。
//   RANGE_CONTINUOUS_MS(30) の数サイクル分の余裕を見て 200ms。
constexpr uint32_t RANGE_PROBE_TIMEOUT_MS = 200;
...
// ★ 2026-09-19: 0.10 -> 0.20。LOG0099/0101 で判明 — 0.10m ではフローがまだ信用できない。
//   PMW3901 の最小合焦距離が約 80mm で、それを下回る間は路面のテクスチャに関係なく
//   ニセのカウントが出る (接地中・モータ回転中に |raw_x| 平均 46 カウント。LOG0094 と
//   LOG0099 で床を変えても同値だったので、テクスチャではなくピントの問題)。
//   0.10m で PosHold を起動すると、その領域の値で位置積分が始まる。
//   0.20m なら合焦距離の 2.5 倍あり、プロペラ後流で床が乱れる高さも抜けている。
constexpr float ALT_AIRBORNE_RISE_M      = 0.20f;
...
// ★ 2026-09-19: 0.30 -> 0.20。上の ALT_AIRBORNE_RISE_M を 0.10 -> 0.20 にしたことの
//   後始末。両者の間 (engage 高度が GROUND_MAX 未満) は「即 latch もせず、+RISE まで
//   上がらないと latch もしない」死に帯で、log_021 の転倒はここに落ちたのが原因だった。
//   0.30/0.20 のままだと 0.29m で POSHOLD に入った場合 0.49m まで latch せず、
//   ALT_TARGET_M=0.50 とほぼ同じ = 死に帯が以前より広がる。2 つを同じ 0.20 に揃えると
//   「engage 点から 0.20m 上がる」か「0.20m より上で engage する」かのどちらかで必ず
//   latch し、帯が消える。地上での測距値 (TSD20 で 0.06〜0.07m) の 3 倍あるので
//   置いたまま誤って latch することはない。
constexpr float ALT_AIRBORNE_GROUND_MAX_M = 0.20f;
...
//  ★ 2026-09-17: 0.5 -> 1.0。本番の定型機動は「前進 v + ヨーレート w」で円を描く
//    ので、v がここでクランプされると **指令より遅く回り、半径が小さくなる**
//    (r = v/w)。ルールの「概ね1.5m以上」を割る側に外れるので、機動速度
//    (position_estimator utils/config.py の COMP_MANEUVER_SPEED = 0.8) より
//    必ず上に取ること。
//  ★ ここは「蓋」であって目標値ではない。上げても普段の巡航速度は変わらない
//    (PC 側の MissionRunner.MAX_VEL = 0.6 が別に効く)。
//  ★ 旋回に要るリーン角は atan(v^2/(R*g))。v=0.8/R=1.5 で 2.5度、v=1.0 で 3.9度 と
//    FLOW_MAX_LEAN(8度) に対して余裕がある。速度側の本当の上限は
//    FLOW_VEL_SANE(3.0m/s) の失探判定。
constexpr float GUIDED_MAX_VEL = 1.0f;
