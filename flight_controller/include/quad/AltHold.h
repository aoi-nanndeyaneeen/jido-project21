...
public:
    ...
    //    50Hz で呼ぶと、同じ climb 値に対して D項が 0 → スパイクを繰り返す。
    ...
    void update(float dt_s, bool enabled, bool armed, bool poshold,
        ...
        if (fresh && dt_s > 0.0f) {
            const bool  sat_hi = (_thr_corr >=  ALT_THR_AUTH - 1e-4f);
            const bool  sat_lo = (_thr_corr <= -ALT_THR_AUTH + 1e-4f);
            const float err    = _vz_tar - climb_mps;
            const bool  integrate = !((sat_hi && err > 0.0f) || (sat_lo && err < 0.0f));

            _thr_corr = constrain(_rate_pid.update(_vz_tar, climb_mps, dt_s, integrate),
                                  -ALT_THR_AUTH, ALT_THR_AUTH);
        }

        // --- 6) 出力 --------------------------------------------------
        _thr_out = constrain(ALT_HOVER_THR + _thr_corr, 0.05f, 1.0f);
    }
