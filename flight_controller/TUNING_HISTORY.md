...
★ 2026-09-19: 追い込み 7 巡目。PITCH -1.45 -> +2.00。
  ★ まず上の 09-15(3)「leanp はトリムでは決まっていない」を撤回する。
    09-10〜09-15 の 6 巡は全部 Madgwick が固定 1ms で積分していた時期の計測で、
    推定角が実角の 0.6〜0.7 倍に縮んでいた (LogLink.h の 2026-09-18 の記述)。
    縮んだものさしで測った感度が巡ごとに食い違うのは当然で、「トリムが効かない」
    のではなく計測系が壊れていた。実測 dt 化 (2026-09-18) 後に測り直して初めて
    筋の通る値が出た。

  ★ 測り方も変えた。従来は fh_leanp を加算していたが、leanp は KI が抱えた
    余剰を含むので毎巡 1.5〜2倍オーバーシュートしていた (その癖を 0.7 倍の
    割引で誤魔化していた)。今回は「機体が実際に止まっていたときの ang_meas」
    を使う。ログの pitch_ang は ang_meas = att.pitch - TRIM なので、
    att.pitch = pitch_ang + TRIM_old で生の姿勢に戻してから読む。

  独立な 2 通りで同じ向き・同じ桁が出た:

    A) LOG0100 (2026-09-19 07:00) POSHOLD 定位置ホバー 22.0秒
         fh_vxc 平均 +0.005 m/s / 正味移動 0.11m = 実質停止
         → フレームは物理的に水平。そのときの ang_meas = +3.49
         → att.pitch = +3.49 + (-1.45) = +2.04   ⇒ TRIM = +2.04
       (参考: fh_leanp = +3.94。従来式ならここを足して +2.49 になるが、
        差の 0.45度 が KI の余剰ぶん = オーバーシュートの正体)

    B) LOG0102 (2026-09-19 07:39) 自動離陸の自由上昇 0.64秒
         PosHold 無出力 (fh_leanr/leanp = 0.00) / スティック中立 /
         ang_meas -0.33 (ほぼ水平指令) なのに前方へ 0.49 m/s^2 で加速。
         姿勢 -0.33度 では 0.06 m/s^2 しか説明できない。
         0.49 m/s^2 = 傾き 2.86度 → att.pitch = -1.78 + 2.86 = +1.08
         ⇒ TRIM = +1.08 (上昇中の抗力が加速度を削るので下限寄り)

  A (22秒の定常。最も直接的) を主、B を下限として +2.00 を採る。

  ★ ロールは据え置き。LOG0100 で fh_leanr +0.26 / roll_ang +0.03 と
    どちらも ±1度 以内。手順どおり 1 本ずつ。
  ★ 症状: これを放置すると自動離陸で 50cm 以上前へ流れる。ホバー中は
    PosHold の KI が打ち消すので見えないが、離陸中は PosHold が
    ALT_AIRBORNE_RISE_M (0.20m) まで切られているため 0.6〜0.7秒ぶん
    野放しになり、PosHold が起動する時点で既に 0.47 m/s 出ている。
    ANGLE で 0.5m まで上げてから POSHOLD に入れる運用では出ない。
  ★ 検証手順: 適用後、地上に置いたときの pitch_ang 表示は約 -2.0度 になる
    (これが正常)。ANGLE で 0.5m・スティック中立で前へ走らなくなれば当たり。
    まだ前へ行くなら足りない / 後ろへ行くなら行き過ぎ。
    次便の POSHOLD で fh_leanp 平均が ±0.5度 に入れば確定。
