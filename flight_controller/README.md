...
| 段階 | 内容 | 高度源 | スロットル |
|---|---|---|---|
| s5a ✅ | PMW3901 搭載 + ログ/表示のみ。符号・軸・de-rotation・`FLOW_PX_PER_RAD` をベンチで確定 | — | 手動 |
| **s5b** | フロー速度・位置ホールド。`SW_AUTO` UP で `POSHOLD`。`vx/vy` → 位置P → 速度PID → 目標リーン角（`ang_tar`）。スティックで速度指令、離すと位置保持 | `FLOW_ASSUMED_HEIGHT_M` 固定（`h` で上書き可） | 手動 |
| s5c | 高度ホールド（スロットルPID）を追加。仮定高度を測距値に差し替え | 距離センサ実測 | 自動 |
| s5d | 高度 + 位置 + ヘディングを1スイッチで同時起動 = 完全自動ホバリング | 距離センサ実測 | 自動 |

s5b 較正済み: `FLOW_PX_PER_RAD = 450`, `FLOW_SIGN_X/Y = -1`。速度ループ／位置ループのゲインは `QuadConfig.h` の `§ 7-2`、飛行中はシリアル `p` メニューの `[i]`(vel P) `[j]`(vel I) `[o]`(pos P)。
