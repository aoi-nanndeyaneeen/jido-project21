...
// 戻り値: 見つかったデバイス数。runMaintenanceAction() が BLE の ActAck に載せる。
static int i2cScan() {
...
//
//  ★ 2026-09-14: 呼び出し元は BLE (handleBleAction) だけになった (IM920は
//    操縦専用に戻したため)。それでも switch を分けたのは、実行内容が
//    シリアル 'r'/'k'/'i' と同じロジックであることを1箇所にまとめて
//    おきたいため (drone_s5.cpp 内の重複を避ける)。
struct ActionResult { uint8_t result; uint8_t imu_ok; uint8_t i2c_found; };

static ActionResult runMaintenanceAction(uint8_t action) {
    switch (action) {
...
// ---- BLE (log_recorder 経由。デバッグ専用) --------------------------------
//  ★ 2026-09-14: 単発メンテナンス指令は BLE 経由に一本化した。IM920 は
//    操縦専用に戻したので、ここには IM920 側の GUIDED_ENABLE / USE_IM920
//    のような前提を一切持ち込まない。IM920 が繋がっていないベンチ上でも
//    IMU校正等を試せることが目的 (詳細は S5Cmd.h の Action コメント参照)。
static uint8_t g_last_ble_action_seq = 0;

static void handleBleAction() {
    if (!g_link_ok) return;
    LogLinkProto::ActReq req;
    if (!LogLink::pollAction(req)) return;
    if (req.action == S5C::ACT_NONE) return;
    if (req.action_seq == g_last_ble_action_seq) return;   // 重複排除
    g_last_ble_action_seq = req.action_seq;

    const ActionResult r = runMaintenanceAction(req.action);
...
void loop() {
    ...
    handleBleAction();                                // BLE 経由のデバッグ指令 (あれば)
