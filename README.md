...
地上局(IM920)を使わず BLE だけでシリアルモニタ風に見たい/PIDリセット・
IMU校正・デバイス確認をデバッグ用に送りたいときは `ble_monitor.py`
(詳細は上記ガイド)。操縦系はこの経路では送れない構造にしてある。
...
★ PIDリセット / IMU再キャリブレーション / デバイス確認 (I2C再走査) は
`console.py` (IM920) には無い。IM920は操縦専用に戻したため、これらは
`ble_monitor.py` (BLE専用) の `P`/`k`/`i` キーを使う (上記参照)。
