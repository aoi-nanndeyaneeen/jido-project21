...
inline void print_PID(float r, float p, float y) {
    Serial.printf("|PIDRoll=%+7.3f |PIDPitch=%+7.3f |PIDYaw=%+7.3f\n", r, p, y);
}

inline void print_MPU(float r, float p, float y, float gr, float gp, float gy) {
    Serial.printf("Roll_Ang:%+7.2f,Pitch_Ang:%+7.2f,Yaw_Ang:%+7.2f,Roll_Gyr:%+7.2f,Pitch_Gyr:%+7.2f,Yaw_Gyr:%+7.2f\n",
                  r, p, y, gr, gp, gy);
}

inline void print_sbus(float Roll_des, float Pitch_des, float Thr_des, float Yaw_des, float Aux1_des, float Aux2_des, float Aux3_des) {
...
inline void print_flightmode(const char* modeName, float bankAngle, unsigned long turnMs) {
    Serial.printf("Mode: %-11s  BankAngle: %+6.1f deg  TurnMs: %5lu ms\n", modeName, bankAngle, turnMs);
}

inline void print_ACC(float ax, float ay, float az) {
    Serial.printf("Acc: ax=%+7.4f ay=%+7.4f az=%+7.4f [g]  |a|=%+7.4f\n",
                  ax, ay, az, sqrtf(ax*ax + ay*ay + az*az));
}

inline void print_timing(unsigned long dt_us) {
