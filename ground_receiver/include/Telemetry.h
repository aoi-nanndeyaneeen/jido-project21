...
public:
    ...
    bool read(Reciver &data) {
        ...
        if (_check_sum + q == 0) {
            ...
            Serial.printf("Telemetry CHECK FAIL! sum:%u q:%u\n", _check_sum, q);
        }
    ...
    bool get_stl() {
        ...
        while (true) {
            ...
            if (_temp.length() >= (sizeof(Reciver) * 2 + 8)) {
                ...
                Serial.println("Telemetry LENGTH FAIL! temp_len: " + String(_temp.length()) + " req: " + String(sizeof(Reciver) * 2 + 8));
                Serial.println("temp content: " + _temp);
            }
        }
    }
};