...
COMP_MANEUVER_SPEED = 0.8     # 機動中の前進速度 [m/s]
COMP_TURN_RADIUS_M  = 1.5     # 指令旋回半径 [m]
...
# 上昇旋回の到達高度 [m]。「ポールの高さ以上」。MISSION_FENCE_Z より十分低くすること
# (超えるとジオフェンスが働いて機動の途中で着陸に落ちる)。
COMP_CLIMB_ALT_M  = 2.20
...
COMP_BUDGET_S = {
    ...
    "circle":     40.0,   # 想定 26s  (2周 23.6s + 開始の間)
    "goto":       20.0,   # 想定  8s  機動どうしの間 (ドリフト 0.5〜1m ぶん)
    "climb":      80.0,   # 想定 61s  (5周 58.9s + 開始の間)
    "figure8":    40.0,   # 想定 26s
    "goto_home":  30.0,   # 想定 12s  ミッションエリア -> 離着陸エリア
    "land":       40.0,   # 想定 11s  (降下 6s + 静止判定 5s)
}
COMP_TARGET_S = 180.0        # 全部終わらせたい時刻 [s] (電池の都合。想定合計は 169s)
# 何をしていてもこの時刻には打ち切って帰投・着陸へ入る [s]。
#  235s (3分55秒) が規定の締切。帰投 12s + 着陸 11s + 余裕 で逆算してある。
COMP_LAND_BY_S = 185.0
# ここまでに帰投が終わらなければ、**その場で** 着陸する [s]。
#  離着陸エリア外の着陸は 800点 -> 300点 に落ちるが、飛んだまま 3分55秒 を
#  迎えると着陸点は 0 になり、後続の倍率ごと失う。
COMP_FORCE_LAND_BY_S = 205.0
