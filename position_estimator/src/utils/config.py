...
# フレーム差分法パラメータ
# ==========================================
# 感度を上げたい（遠距離・ゆっくりした動き）→ DIFF_THRESHOLDとMIN_AREA_PXを下げる
# ノイズが多い（誤検出が多い）→ 両方を上げる

DIFF_THRESHOLD = 10    # 差分2値化の閾値（デフォルト25→15に変更）
MIN_AREA_PX    = 20    # 検出とみなす最小輪郭面積 [px²]（デフォルト100→40に変更）
BLUR_KERNEL    = 5     # ガウシアンブラーのカーネルサイズ（奇数のみ）
MORPH_KERNEL   = 5     # モルフォロジーのカーネルサイズ
...
# カメラフレームレート
# ==========================================
CAMERA_FPS = 60   # 要求FPS（一般的なデフォルト30fpsの2倍）
                  # ハードウェアが非対応の場合は自動で上限にフォールバックする