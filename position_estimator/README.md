...
`MISSION_AUTOSTART = True`（既定）なら **[M] を押す必要はありません**。起動した時点でミッションは待機（ARMING）に入り、`REQ_HOLD` を流し続けます。プロポの `SW_HOVER` を上げて離陸し、機体が GUIDED に入った瞬間に自動で離陸フェーズへ進みます。本番で離陸後に PC を触れないための既定値です。
