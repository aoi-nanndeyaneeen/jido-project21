...
- `Diff_X/Y/Norm` = カメラ実測 − 機体のフロー推定。機体は光学フローの積分で自分の位置を持っており、これは必ず流れる。その流れた量そのもの。
  - 原点合わせは「カメラとフローが同時に有効になった最初の瞬間」に1回だけ行う（`Drone_X/Y` はその補正後）。生値の `Fh_PosN/Fh_PosE` も残してあるので、後から別基準で取り直せる。
  - `n`(北) をフィールド `+y`(奥)、`e`(東) を `+x`(右) に対応させている。これは「機首をフィールド奥へ向けて置く」運用前提（`YAW_INITIAL_ALIGN_DEG=0`）そのもので、前提が崩れていればズレに回転として現れる。
